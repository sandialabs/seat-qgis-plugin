...
def plot_test_obstacle_locations(Obstacles):
    ...
    for ic, obstacle in enumerate(Obstacles.keys()):
        ...
        ax.text(Obstacles[obstacle][0,0], Obstacles[obstacle][0,1], f'{obstacle}', fontsize=8)
        ax.text(Obstacles[obstacle][1,0], Obstacles[obstacle][1,1], f'{obstacle}', fontsize=8)
    fig.tight_layout()
    return fig
...
def create_power_heatmap(DEVICE_POWER):
    ...
    cmap = ListedColormap(plt.get_cmap('turbo')(np.linspace(0.1, 1, 256)))  # skip too light colors
    # norm = plt.Normalize(DEVICE_POWER['Power [W]'].min(), DEVICE_POWER['Power [W]'].max())
    norm = plt.Normalize(0.9*DEVICE_POWER['Power [W]'].min()*1e-6, DEVICE_POWER['Power [W]'].max()*1e-6)
...
def read_power_file(datafile):
    ...
    for line in inf: #iterate through each line
        if re.match('Iteration:', line):
            ...
            power = float(line.split('=')[-1].split('W')[0].strip()) #extract float variable from line
...
def calculate_power(power_files, bc_file, save_path=None):
    ...
    fig,AX = plt.subplots(np.round(foo).astype(int), np.ceil(foo).astype(int), sharex=True, sharey=True, figsize=(12,10))
    nr,nc = AX.shape
    AX = AX.flatten()
#    mxy = roundup(np.log10(Power_Scaled.max().max())) + 5
    ...
    for ic in range(Power_Scaled.shape[1]):
        # fig,ax = plt.subplots()
        AX[ic].bar(np.arange(np.shape(Power_Scaled)[0])+1, np.log10(Power_Scaled[:,ic]), width=1, edgecolor='black')
#        AX[ic].text(Power_Scaled.shape[0]/2, mxy-1, f'{datafiles[ic]}', fontsize=8, ha='center', va='top')
    ...
    fig,ax = plt.subplots(figsize=(9,6))
    ax.bar(np.arange(np.shape(Power_Scaled)[0])+1, np.log10(np.sum(Power_Scaled, axis=1)), width=1, edgecolor='black')
    ax.set_xlabel('Obstacle')
    ax.set_ylabel('Power [$log_{10}(Watts)$]')
    ...
    power_device_configuration_file = [s for s in os.listdir(power_files) if (s.endswith('.pol') | s.endswith('.Pol') | s.endswith('.POL'))]
    if len(power_device_configuration_file) > 0:
        ...
        Obstacles = read_obstacle_polygon_file(os.path.join(power_files, power_device_configuration_file[0]))
        ...
        for device in DeviceindexDF.index.values:
            ax.plot(DeviceindexDF.loc[device, 'X'], DeviceindexDF.loc[device, 'Y'], '.', alpha=0)
            ax.text(DeviceindexDF.loc[device, 'X'], DeviceindexDF.loc[device, 'Y'], device, fontsize=8)
...
#        mxy = roundup(np.log10(Devices.max().max())) + 5
        ...
        for ic, col in enumerate(Devices.columns):
            # fig,ax = plt.subplots()
            AX[ic].bar(np.arange(np.shape(Devices[col])[0])+1, np.log10(Devices[col].to_numpy()), width=1.0, edgecolor='black')
