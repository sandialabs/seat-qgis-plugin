...
def calculate_shear_stress_stressors(fpath_nodev, 
    ...
    if gridtype=='structured':
        ...
        rx = xcor
        ry = ycor
        listOfFiles = [tau_diff, mobility_parameter_nodev, mobility_parameter_dev, mobility_parameter_diff, mobility_classification, receptor_array, tau_combined_dev, tau_combined_nodev]
    else: # unstructured
...
def run_shear_stress_stressor(
    ...
    if not((receptor_filename is None) or (receptor_filename == "")):
        ...
        use_numpy_arrays = [numpy_arrays[0], numpy_arrays[3], numpy_arrays[4], numpy_arrays[5], numpy_arrays[6], numpy_arrays[7]]
    else:
        numpy_array_names = ['calculated_stressor.tif',
                            'tau_with_devices.tif',
                            'tau_without_devices.tif']
